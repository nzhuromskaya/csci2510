...
var startScene = Engine.Base.Scene.parse(Scenes.StartScene);
SceneManager.addScene(startScene);
SceneManager.currentScene = "StartScene"; //Setup event handling
...
function mousedown(event) {
  if (Input.mouseButtons[event.button] != true) Input.mouseButtonsDown[event.button] = true;
  Input.mouseButtons[event.button] = true;
}

function mouseup(event) {
  if (Input.mouseButtons[event.button] != false) Input.mouseButtonsUp[event.button] = true;
  Input.mouseButtons[event.button] = false;
}

function keypress(event) {} //console.log(`Modifier keys: Control: ${event.ctrlKey}, Alt: ${event.altKey}, Shift: ${event.shiftKey}, Meta Key: ${event.metaKey}`);
...
window.onresize = resizeCanvas; // So we fire it manually...
